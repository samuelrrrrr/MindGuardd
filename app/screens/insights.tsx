...
import { Card, Pill, Bar, LineChart } from '../../components/UI';
    ...
    >
      ...
      >
        <View style={{ paddingHorizontal: 22 }}>
          <Pill label="Weekly Analysis" color={C.purpleLight} bg="rgba(162,155,254,0.2)" />
      ...
      <View style={styles.content}>
        ...
        >
          <Text style={styles.aiQuote}>
            "I noticed you've been feeling a bit stressed lately, particularly during your evening reflections."
          </Text>
          <Text style={styles.aiDesc}>
            Your resilience levels have stayed consistent, but I see a dip in focus during mid-week. This seems to correlate with shorter sleep cycles. A 5-minute breathing break at 3 PM could help!
          </Text>
        </LinearGradient>
        ...
        <Card>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.75}
          >
            <Text style={[styles.focusBtnText, { color: C.purple }]}>Boost focus now</Text>
          </TouchableOpacity>
        </Card>

        <Card>
          <Text style={[styles.cardTitle, { marginBottom: 10 }]}>Mood Trajectory</Text>
          <LineChart data={chartData} color={C.purple} h={68} />
          <View style={[styles.rowBetween, { marginTop: 8 }]}>
            {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((d) => (
              <Text key={d} style={styles.muted}>{d}</Text>
            ))}
          </View>
        </Card>
      </View>
...
const styles = StyleSheet.create({
  ...
  aiQuote: { fontSize: 15, fontWeight: '700', color: '#fff', lineHeight: 24, marginBottom: 12, fontStyle: 'italic' },
  aiDesc: { fontSize: 13, color: 'rgba(255,255,255,0.6)', lineHeight: 22 },

  miniBarRow: { flexDirection: 'row', alignItems: 'flex-end', height: 48, gap: 4, marginBottom: 6 },
  miniBar: { flex: 1, borderTopLeftRadius: 4, borderTopRightRadius: 4 },

  focusBtn: { backgroundColor: C.purplePale, borderRadius: 99, paddingVertical: 10, alignItems: 'center' },
